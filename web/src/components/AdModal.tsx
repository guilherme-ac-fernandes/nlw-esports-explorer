...
function AddModal() {
  ...
  const [weekDays, setWeelDays] = useState<string[]>([]);
  ...
  const handleCreateAd = async (event: FormEvent) => {
    ...
    // Exemplo de biblioteca para validação em React => React Hook Form (Schema validation => Zod)
    if (!data.name) {
      return;
    }
    ...
    alert('Anúncio criado com sucesso');
      ...
      console.error(err)
      alert('Erro ao criado o anúncio');
    }
        ...
        <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              {games.map((game) => <option key={ `OPT-${game.id}` } value={game.id}>{game.title}</option> )}
            </select>
          </div>
          ...
          <div className="flex gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Toggle.Root type='multiple' className="grid grid-cols-4 gap-2" onValueChange={setWeelDays} value={weekDays}>
